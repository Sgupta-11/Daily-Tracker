...
{ pct: 50, text: "Halfway. The version of you that gave up doesn't exist anymore. { pct: 55, text: "55% there. You're genuinely built different now." },
...
.filter(w => !isNaN(w));
const latestWeight = weight ? parseFloat(weight) : (allWeights.length > 0 ? allWeights[allW
...
const progressColor = progressPct >= 75 ? "#4ade80" : progressPct >= 40 ? "#facc15" : "#60a
// Calories
const cal = parseInt(calories) || null;
const calColor = cal === null ? "#888" : cal <= CALORIE_TARGET ? "#4ade80" : cal <= CALORIE
...
return count;
const checks = [
{ id: "gym", emoji: " { id: "steps", emoji: " { id: "protein", emoji: " { id: "sleep", emoji: " ", label: "Gym", sub: "Showed up = counts" },
", label: "10k Steps", sub: "Daily step goal" },
", label: "Protein", sub: "~150g target" },
", label: "Sleep supps", sub: "Mag + Ashwagandha" },
];
const todayChecked = checks.filter(c => today[c.id] === true).length + (calUnder === true ?
function getStatus() {
...
<link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&f
"24px
{/* HEADER */}
<div style={{ background: "#0f0f0f", borderBottom: "1px solid #1c1c1c", padding: <div style={{ maxWidth: 480, margin: "0 auto" }}>
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-s
<div>
<div style={{ fontSize: 10, letterSpacing: 3, color: "#444", textTransform: "up
<div style={{ fontSize: 22, fontWeight: 700, letterSpacing: -0.5 }}>
{new Date().toLocaleDateString("en-IN", { weekday: "long", day: "numeric", mo
</div>
</div>
<div style={{ textAlign: "right" }}>
<div style={{ fontSize: 10, color: "#444", marginBottom: 2, letterSpacing: 1 }}
<div style={{ fontSize: 34, fontWeight: 700, lineHeight: 1, color: streak > 0 ?
...
<span style={{ color: status.color, fontWeight: 600, fontSize: 13 }}>{status.text
...
<strong>Two rough days in a row.</strong> Don't let it become three. Next me
</div>
)}
</div>
</div>
...
<div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-s
<div>
<div style={{ fontSize: 10, letterSpacing: 2, color: "#444", textTransform: "up
<div style={{ fontSize: 28, fontWeight: 700, color: progressColor, lineHeight:
{progressPct}%
</div>
</div>
<div style={{ textAlign: "right" }}>
<div style={{ fontSize: 11, color: "#555" }}>{latestWeight} kg → {GOAL_WEIGHT}
<div style={{ fontSize: 13, fontWeight: 600, color: "#888", marginTop: 2 }}>
{Math.max(0, (latestWeight - GOAL_WEIGHT)).toFixed(1)} kg to go
</div>
<div style={{ fontSize: 10, color: "#444", marginTop: 2 }}>lost {lost.toFixed(1
</div>
</div>
{/* Progress bar */}
...
<>
{/* WEIGHT LOG */}
<div style={{ fontSize: 10, color: "#3a3a3a", letterSpacing: 2, textTransform: "u
<div style={{
...
{[{ id: "gym", emoji: " ", label: "Gym", sub: "Showed up = counts" }, { const val = today[q.id];
return (
<div key={q.id} style={{
flex: 1,
background: val === true ? "rgba(74,222,128,0.07)" : val === false ? "rgb
border: `1px solid ${val === true ? "rgba(74,222,128,0.22)" : val === fal
...
<YesNo value={val} onYes={() => save({ [q.id]: true })} onNo={() => save(
</div>
);
})}
</div>
...
<div style={{
background: "#0f0f0f",
border: `1px solid ${cal === null ? "#1c1c1c" : cal <= CALORIE_TARGET ? "rgba(7
...
<div style={{ fontSize: 11, color: "#444" }}>Target: {CALORIE_TARGET} kca
</div>
</div>
<div style={{ display: "flex", alignItems: "center", gap: 10 }}>
...
{cal !== null && (
<>
<div style={{ marginTop: 10, height: 3, background: "#1a1a1a", borderRadius
<div style={{
height: "100%", width: `${Math.min((cal / (CALORIE_TARGET + 500)) * 100
background: calColor, borderRadius: 2, transition: "width 0.3s ease",
}} />
</div>
<div style={{ fontSize: 11, color: calColor, marginTop: 5, textAlign: "righ
{cal <= CALORIE_TARGET ? `${CALORIE_TARGET - cal} kcal under ✓` : `${cal
</div>
</>
)}
</div>
...
{[{ id: "protein", emoji: " ", label: "Protein", sub: "~150g target" }, const val = today[q.id];
return (
<div key={q.id} style={{
background: val === true ? "rgba(74,222,128,0.06)" : val === false ? "rgb
border: `1px solid ${val === true ? "rgba(74,222,128,0.2)" : val === fals
...
<div>
<div style={{ fontWeight: 600, fontSize: 14 }}>{q.label}</div>
<div style={{ fontSize: 11, color: "#444", marginTop: 1 }}>{q.sub}</d
</div>
</div>
<YesNo value={val} onYes={() => save({ [q.id]: true })} onNo={() => save(
</div>
...
placeholder="Any slip today? e.g. ordered chole bhature, skipped gym..." rows
style={{ width: "100%", background: "#0f0f0f", border: "1px solid #1c1c1c", b
...
<div style={{ marginTop: 14, padding: "12px 16px", background: "#0a0a0a", border:
...
{/* Weight chart */}
<div style={{ fontSize: 10, color: "#3a3a3a", letterSpacing: 2, textTransform: "u
<div style={{ background: "#0f0f0f", border: "1px solid #1a1a1a", borderRadius: 1
<div style={{ display: "flex", alignItems: "flex-end", gap: 6, height: 60, just
...
const hasW = !isNaN(w);
const barH = hasW ? Math.max(8, Math.min(60, ((START_WEIGHT - w + 5) return (
<div key={key} style={{ flex: 1, display: "flex", flexDirection: "column"
{hasW && <div style={{ fontSize: 9, color: "#555", fontFamily: "'DM Mon
<div style={{
...
<div style={{ fontSize: 9, color: isToday ? "#fff" : "#444" }}>{days[da
</div>
);
})}
</div>
...
const c = parseInt(d.calories);
const calVal = c ? c <= CALORIE_TARGET : d.calUnder;
const yesItems = [d.gym, d.steps, d.protein, d.sleep, calVal].filter(v => v =
const hasData = [d.gym, d.steps, d.protein, d.sleep, d.calories, d.calUnder].
const barColor = score >= 0.8 ? "#4ade80" : score >= 0.5 ? "#facc15" : "#f871
return (
<div key={key} style={{
background: "#0f0f0f", border: `1px solid ${isToday ? "#2a2a2a" : "#16161
borderRadius: 11, padding: "11px 14px",
display: "flex", alignItems: "center", gap: 10,
}}>
overfl
<div style={{ width: 36, textAlign: "center", flexShrink: 0 }}>
<div style={{ fontSize: 9, color: "#444" }}>{days[dateObj.getDay()]}</d
<div style={{ fontSize: 16, fontWeight: 700, color: isToday ? "#fff" :
</div>
<div style={{ flex: 1 }}>
<div style={{ height: 4, background: "#1a1a1a", borderRadius: 2, {hasData && score !== null && (
<div style={{ height: "100%", width: `${score * 100}%`, background:
)}
</div>
{c ? <div style={{ fontSize: 9, color: c <= CALORIE_TARGET ? "#4ade80"
</div>
<div style={{ display: "flex", gap: 4 }}>
{[{ e: " ", v: d.gym }, { e: " ", v: d.steps }, { e: " ", v: d.prot
<span key={idx} style={{ fontSize: 12, opacity: item.v === true ? 1 :
))}
</div>
<div style={{ width: 24, textAlign: "right", flexShrink: 0 }}>
{hasData ? <span style={{ fontSize: 11, fontFamily: "'DM Mono', monospa
...
{/* Breakdown */}
<div style={{ marginTop: 14, background: "#0f0f0f", border: "1px solid #1a1a1a",
<div style={{ fontSize: 10, color: "#3a3a3a", letterSpacing: 2, textTransform:
{[
{ emoji: " ", label: "Gym", key: "gym" },
...
if (item.key === "cal") { const c = parseInt(d.calories); return c ? return d[item.key];
});
...
<span style={{ fontSize: 12, color: "#555", width: 76 }}>{item.label}</sp
<div style={{ flex: 1, height: 4, background: "#1a1a1a", borderRadius: 2,
<div style={{ height: "100%", width: `${ratio * 100}%`, background: rat
</div>
<span style={{ fontSize: 10, color: "#444", fontFamily: "'DM Mono', monos
</div>
);
})}
</div>
